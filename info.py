...
(
    (
        '画像ビューアで画像フリップ時に内部移動が反転してしまうバグを修正。',
        'selectionSupporterにアニメーションフィルタを追加。',
        '一部モジュールのドキュメント更新と、モジュールをローカルインポート化。',
    ),
)
),
...
(
    (
        'factory内やコンストラクタでカレントファイルの扱いを、特殊リンクファイルになるよう仕様変更。',
        '合わせて各種エクスポーターとファイルブラウザを特殊リンクに対応。',
    ),
)
),
...
(
    (
        'factoryUIの構成を大幅変更。',
        'factoryUIのファイルビューワの表示スタイルの刷新＋フィルタ機能追加。',
    ),
)
),
...
('2.0.3.4',
(
    'gadgets.nodeRenamerのノード種別判別アルゴリズムを一部更新',
)
),
...
('2.0.3.2',
(
    'displayLayerViewerの描画まわりを更新。（レイヤ表示により色が変化）',
)
),
...
('2.0.3.0',
(
    '関数ドキュメントを解析して整形文に変換する関数を独立したモジュールへ移動。',
)
),
