...
r'''
    @file     ikFkChain.py
    @brief    IKスプラインとFK調整コントローラを付加するリグを提供するモジュール
    @class    Option : IKコントローラの数を決めるオプションを定義するクラス。
    @class    JointCreator : ジョイント作成用のクラス。
    @class    RigCreator : IKスプラインとFK調整コントローラ機能を作成する。
    @date        2017/09/17 19:56[Eske](eske3g@gmail.com)
    @update      2017/09/17 20:00[Eske](eske3g@gmail.com)
...
class Option(rigScripts.Option):
    r'''
        @brief       IKコントローラの数を決めるオプションを定義するクラス。
...
class JointCreator(rigScripts.JointCreator):
    ...
    def process(self):
        r'''
            @brief  ジョイント作成を行う。始点と終点のジョイントのみ作成する。
            @return None
        '''
...
class RigCreator(rigScripts.RigCreator):
    r'''
        @brief       IKスプラインとFK調整コントローラ機能を作成する。
    ...
    def process(self):
        ...
        def blendScaleAttr(srcAttrA, srcAttrB, dstAttr):
            r'''
                @brief  ２つのスケールアトリビュートをブレンドする。
                @param  srcAttrA : [str]入力スケールアトリビュートA
                @param  srcAttrB : [str]入力スケールアトリビュートB
                @param  dstAttr : [str]ブレンドされる対象スケールアトリビュート
                @return None
            '''
        ...
        # grow system append
        param_grow = param.addFloatAttr('grow', default = 1)
        dist_grow = dist_node.addFloatAttr('grow', default = 1)
        for sprigjnt in spline_rigJnt['ikJnt']: 
            inputnode = sprigjnt.attr('tx').source(p = True)
            if inputnode is None: 
                continue
            mdl = func.createUtil('multDoubleLinear')
            inputnode >> mdl.attr('i1')
            dist_grow >> mdl.attr('i2')
            mdl.attr('o') >> sprigjnt.attr('tx')

        param_grow >> dist_grow
        # grow system append
        ...
        # spline_all_space.fitTo(spline_rigJnt['fkJnt'][0])
        ...
        #grow_append
        func.connectKeyableAttr(spline_rigJnt['fkJnt'][-1], spline_rigJnt['combJnt'][-1])
        # =====================================================================
        # /////////////////////////////////////////////////////////////////////
        #                                                                    //
        # /////////////////////////////////////////////////////////////////////
        ...
        pre_vec = func.Vector(fk_ctrls[0].position())
